...
function HomePage() {
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        const [homeRes, packageRes, mediaRes] = await Promise.all([
          homeApi.getPublic(),
          packageApi.getPublic(),
          mediaApi.getPublic({ category: "home" })
        ]);

        setHomeContent(homeRes.data.data);
  ...
  return (
    <div>
      ...
      {/* ─── Hero Section ─── */}
      <section className="relative flex min-h-[95vh] items-end overflow-hidden px-8 pb-32">
        {heroIsVideo ? (
        ...
        <div className="absolute top-20 right-20 h-64 w-64 rounded-full bg-[var(--accent-pink)]/8 blur-[80px] animate-float-gentle" />
        <div className="absolute bottom-32 right-40 h-48 w-48 rounded-full bg-[var(--accent-purple)]/6 blur-[60px] animate-float-gentle" style={{ animationDelay: "2s" }} />
        <div className="absolute top-1/2 left-1/3 h-32 w-32 rounded-full bg-[var(--accent-light)]/5 blur-[50px] animate-float-gentle" style={{ animationDelay: "4s" }} />
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl">
          ...
          <HeroWordStagger
            ...
            className="font-heading text-5xl uppercase text-[var(--text-primary)] drop-shadow-none md:max-w-4xl md:text-7xl"
          />
          <p className="mt-8 max-w-2xl text-lg leading-relaxed text-[var(--text-secondary)] animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
            {homeContent?.heroSubtitle || "Freelance wedding cinematographer and portrait photographer. Every frame tells your story."}
          </p>

          <div className="mt-12 flex flex-wrap gap-5 animate-fade-in-up" style={{ animationDelay: "0.5s" }}>
            <Link to="/booking">
              <Button className="px-8">{homeContent?.ctaText || "Book Arun"}</Button>
            </Link>
            <Link to="/portfolio">
              <Button variant="outline" className="px-8">Explore Portfolio</Button>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="section-space px-8">
        <div className="mx-auto max-w-7xl">
          <FadeUpReveal>
            <SectionHeading
              eyebrow="Services"
              title="What I Do"
              description={homeContent?.aboutSnippet}
            />
              ...
              {packageRows.map((item, index) => {
                ...
                return (
                  <div
                    ...
                    className="sticky shadow-[0_8px_40px_rgba(31,31,31,0.04)] transition-all duration-500 w-full rounded-[2rem] overflow-hidden border border-[var(--card-border)] hover-lift"
                    style={{ top: `${140 + index * 50}px`, zIndex: index + 10 }}
                  >
                    <div className={`relative group flex flex-col md:flex-row w-full min-h-[320px] ${bgColor} overflow-hidden`}>
                      ...
                      <div className="w-full md:w-[55%] p-8 md:p-12 flex flex-col justify-center">
                        <div className="flex flex-col xl:flex-row xl:justify-between xl:items-start gap-6">
                          <div>
                            <h3 className="font-heading text-4xl md:text-5xl uppercase tracking-wider text-[var(--text-primary)]">{item.name}</h3>
                            <p className="mt-3 font-heading text-xl md:text-2xl font-bold text-[var(--accent-pink)]">{item.priceDisplay}</p>
                          </div>
                        ...
                        <div className="w-full h-px bg-gradient-to-r from-[var(--accent-pink)]/15 via-[var(--accent-purple)]/10 to-transparent my-6 md:my-8" />

                        <ul className="space-y-4 text-[var(--text-secondary)] text-base md:text-lg relative z-30">
              ...
              {gallery.map((image, index) => (
                ...
                >
                  <img
                    ...
                    className="h-[22rem] w-full object-cover object-[center_30%] transition duration-500 group-hover:scale-[1.05] md:h-[32rem]"
                  />
                </div>
              ))}
            </div>
      ...
      {/* ─── Inquiry Form ─── */}
      <section className="section-space px-8 pb-32">
        <div className="relative mx-auto grid max-w-7xl gap-14 rounded-[2.5rem] border border-[var(--card-border)] bg-gradient-to-br from-[var(--surface)] via-[var(--surface)] to-[var(--bg-secondary)] p-12 lg:grid-cols-2 overflow-hidden">
          {/* Decorative background orbs */}
          <div className="absolute -top-20 -right-20 h-60 w-60 rounded-full bg-[var(--accent-pink)]/5 blur-[80px] animate-float-gentle" />
          <div className="absolute -bottom-16 -left-16 h-48 w-48 rounded-full bg-[var(--accent-purple)]/5 blur-[60px] animate-float-gentle" style={{ animationDelay: "3s" }} />
          ...
          <div className="relative z-10">
            ...
            <h3 className="font-heading text-6xl uppercase tracking-wider text-[var(--text-primary)]">Get in Touch</h3>
          ...
          <form className="relative z-10 space-y-6" onSubmit={handleInquirySubmit}>
            <div className="grid gap-6 md:grid-cols-2">
